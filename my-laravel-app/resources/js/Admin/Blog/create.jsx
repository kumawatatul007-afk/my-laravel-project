                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-row">
                        <div className="form-group">
                            ...
                            <input className={`form-input${errors.author ? ' err' : ''}`} value={data.author} onChange={e => setData('author', e.target.value)} placeholder="Author name" />
                            {errors.author && <p className="form-error">{errors.author}</p>}
                        </div>
                        <div className="form-group">
                            <label className="form-label">Category</label>
                            <input className="form-input" value={data.category} onChange={e => setData('category', e.target.value)} placeholder="e.g. Design, Development" />
                        </div>
                    </div>

                    <div className="form-group">
                        <label className="form-label">Excerpt</label>
                        <textarea className="form-input" rows={3} value={data.excerpt} onChange={e => setData('excerpt', e.target.value)} placeholder="Short description..." style={{ resize: 'vertical' }} />
                    </div>

                    <div className="form-group">
                        <label className="form-label">Content</label>
                        <textarea className={`form-input${errors.content ? ' err' : ''}`} rows={8} value={data.content} onChange={e => setData('content', e.target.value)} placeholder="Full blog content..." style={{ resize: 'vertical' }} />
                    ...
                    <div className="form-row">
                        ...
                        <div className="form-group">
                            ...
                            <select className={`form-input${errors.status ? ' err' : ''}`} value={data.status} onChange={e => setData('status', e.target.value)}>
